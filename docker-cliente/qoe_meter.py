...
def get_url_video(escolha_video):
    dominio = "http://service-nginx-qoe:5215"
    url_video = dominio+"/hls/playlist_master.m3u8"

    if escolha_video == 0:
        url_video = dominio+"/hls/video_60_180p.m3u8"        
    elif escolha_video == 1:
        ...
        url_video = dominio+"/hls/playlist_master.m3u8"        
    elif escolha_video == 8:
        url_video = dominio+"/dash/manifest.mpd"
    else:
        url_video = dominio+"/dash/manifest.mpd"
    return url_video
...
while cont < quant:
    ...
    if (not video_assistido):
        try:
            ...
            escolha_video = random.randint(0, 8)
